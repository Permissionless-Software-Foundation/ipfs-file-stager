...
describe('#round-robin-facilitator-client', () => {
  it('rejects non-array or empty entries', () => {
    try {
      // eslint-disable-next-line no-new
      new RoundRobinFacilitatorClient()
      assert.fail('expected throw')
    } catch (e) {
      assert.include(e.message, 'at least one facilitator')
    }
    try {
      // eslint-disable-next-line no-new
      new RoundRobinFacilitatorClient([])
  ...
  it('throws the last error when every verify() fails', async () => {
    const uut = new RoundRobinFacilitatorClient([
      makeEntry('A', { verify: async () => { throw new Error('a') } }),
      makeEntry('B', { verify: async () => { throw new Error('b') } })
    ])
    try {
      await uut.verify({ x402Version: 2 }, { network: 'n', scheme: 's' })
      assert.fail('expected throw')
    } catch (e) {
  ...
  it('tolerates missing kinds and skips non-array signers in getSupported', async () => {
    const uut = new RoundRobinFacilitatorClient([
      makeEntry('A', {
        getSupported: async () => ({
          kinds: undefined,
          extensions: [],
          signers: { 'eip155:8453': 'not-array' }
        })
      })
    ])
    const s = await uut.getSupported()
