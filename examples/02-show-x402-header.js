/**
 * x402 v2 — fetch a protected URL, decode PAYMENT-REQUIRED.
...
const url = process.env.X402_SAMPLE_URL || 'http://localhost:5040/ipfs'
...
async function main () {
  ...
  console.log(
    `content-type: ${headerString(res.headers, 'content-type') ?? '(none)'}`
  )
  console.log(
    `access-control-expose-headers: ${
      headerString(res.headers, 'access-control-expose-headers') ?? '(none)'
    }`
  )
  ...
  if (!paymentRequiredRaw) {
    console.error(
      '\nNo PAYMENT-REQUIRED header on this response. Is X402_ENABLED=true?'
    )
    process.exit(1)
  }
